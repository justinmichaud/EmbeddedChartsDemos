...
const PROD_URL = process.env.PROD_URL || 'http://localhost:5174/iframe.html';
...
test('cpu profile (prod build): steady-state updates', async ({ page }) => {
  await page.goto(`${PROD_URL}?testMode=true`);
  ...
  const profilePath = path.resolve('cpu-profile-prod.cpuprofile');
  ...
  for (const [id, sUs] of selfBy) {
    ...
    const file = (cf.url || '').split('/').slice(-1).join('/');
