...
// Vite's BASE_URL is '/' in dev and '/EmbeddedChartsDemos/' in the deployed
// build. Strip the trailing slash so `buildPrefix + '/iframe.html'` is a valid
// path in both (avoids the '//iframe.html' protocol-relative bug in dev).
const buildPrefix = import.meta.env.BASE_URL.replace(/\/$/, '');
