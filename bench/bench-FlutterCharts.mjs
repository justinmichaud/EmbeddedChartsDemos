#!/usr/bin/env node
// Benchmark launcher for FlutterCharts (native macOS desktop app).
//
// Prereq: build the profile binary once with FPS instrumentation:
//   cd FlutterCharts && flutter build macos --profile
// (The FPS emitter is gated on the BENCH_FPS=1 env var this launcher sets.)
//
...
// Find the built profile .app executable.
...
// Reset persisted settings so the run uses the app defaults.
const settingsFile = path.join(os.homedir(), 'Library/Application Support/FlutterCharts/settings.json');
try {
